...
export default function DomainVerification({ domainData }: DomainVerificationProps) {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (result?.data?.success) {
        ...
        toast({ title: 'Verification failed', description: result?.data?.error, variant: 'destructive' })
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-4 max-w-4xl mx-auto">
      ...
      <div className="flex justify-between">
        <Button variant="outline" onClick={() => router.push('/onboarding/email-setup')}>Back</Button>
        <div className="flex gap-2">
          <Button onClick={() => router.push('/onboarding/mailing-address')} variant="outline">
            Skip
          </Button>
