...
export const sendBroadcastTask = task({
  ...
  run: async (
    ...
    const { data: emailData, error: emailError } = await logger.trace(
      "Fetch email content",
      async () => {
    ...
    const { data: individualContacts, error: individualContactsError } =
      await logger.trace("Fetch individual contacts", async () => {
        return await supabase
          .from("contacts")
    ...
    if (individualContactsError)
      throw new Error(
        `Failed to fetch individual contacts: ${individualContactsError.message}`,
      );

    // Fetch contacts from listIds
    const { data: listContacts, error: listContactsError } = await logger.trace(
      "Fetch list contacts",
      async () => {
        return await supabase
          .from("list_contacts")
    ...
    if (listContactsError)
      throw new Error(
        `Failed to fetch list contacts: ${listContactsError.message}`,
      );
    ...
    const { error: updateError } = await logger.trace(
    ...
    if (updateError)
      throw new Error(
        `Failed to update broadcast status: ${updateError.message}`,
      );
